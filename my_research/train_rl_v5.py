...
def train():
    ...
    target_model = DQNAgentLSTM().to(DEVICE)
    ...
    for episode in range(EPISODES):
        ...
        hidden = None  # LSTMの隠れ状態をリセット
        total_reward = 0

        for step in range(MAX_STEPS):
            action, action_idx, hidden = model.get_action(
                state, hidden, epsilon
            )
    ...
    torch.save(model.state_dict(), '/work/my_research/rl_model_v5.pth')
    print("学習完了: rl_model_v5.pth を保存しました")
