...
def visualize_trajectory(n_episodes=5):
    ...
    for ep in range(n_episodes):
        ...
        for step in range(100):
            # epsilon=0で完全にモデルの判断で動く
            action, q = model.get_action(state, epsilon=0.0)
    ...
    plt.suptitle('Agent Trajectory (epsilon=0)', fontsize=14)
    plt.tight_layout()
    plt.savefig('/work/my_research/trajectory.png', dpi=150, bbox_inches='tight')
