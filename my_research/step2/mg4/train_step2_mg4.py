...
train_step2_mg3.py からの変更点:
    scale_vision (v*2-1 → [-1,1]) を追加。

元論文の exp3 (MotionGenerationPredictionRunner) は loader.py で
    batch['self_vision'] = util.scale_vision(...)
としており、飯塚モデルの encoder も [-1,1] 入力前提で学習済み。
mg3 では [0,1] のまま渡していたため encoder が学習時と異なる入力を受けていた。
mg4 では全ての視覚テンソルに scale_vision を適用し、論文準拠にする。

その他は mg3 と同一:
    - MG2 (ov_enc 入力版 Motion Generator)
    - マスク戦略: t=0 のみ 0、t>0 は 1.0
    - Q 値拡張

保存先: results/mg4_YYYYMMDD_HHMMSS/

実行コマンド:
    MESA_GL_VERSION_OVERRIDE=3.3 xvfb-run python3 train_step2_mg4.py
"""
...
# mg4/ → step2/ → my_research/ → superposition/
...
from util import scale_vision  # 元論文準拠: v*2-1 → [-1,1]
...
RUN_DIR         = os.path.join(STEP2_DIR, 'results', f'mg4_{_TIMESTAMP}')
MODEL_SAVE_PATH = os.path.join(RUN_DIR, 'model_mg4.pth')
...
def preprocess_vision(v_raw):
    ...
    scale_vision を適用して元論文の encoder 入力形式に合わせる。
    """
    v_chw = v_raw.transpose(2, 0, 1)           # (C, H, W) [0,1]
    v_scaled = scale_vision(v_chw)              # (C, H, W) [-1,1]
    return torch.FloatTensor(v_scaled).unsqueeze(0).to(DEVICE)
...
def check_encoder_similarity(net, n=100):
    ...
    with torch.no_grad():
        for _ in range(n):
            # scale_vision で [-1,1] の乱数入力（encoder 入力分布に合わせる）
...
def train():
    ...
    log(f"scale_vision     : 有効 (v*2-1 → [-1,1], 元論文準拠)", log_file)
    log(f"MG 入力          : ov_enc (元論文準拠)", log_file)
    log(f"マスク戦略       : p_mask_other=(step==0? 0 : 1.0) (MotionGenerationPredictionRunner準拠)", log_file)
    ...
    log(f"\n=== train_step2_mg4 学習開始 (scale_vision 有効) ===\n", log_file)
    ...
    for episode in range(EPISODES):
        ...
        for step in range(MAX_STEPS):
            ...
            # t=0 のみマスクなし、t>0 は other を完全マスク（元論文準拠）
            p_mask_other = 0.0 if step == 0 else 1.0
            pred, ss, os_ = net(x, q_self, 0.0, p_mask_other)
            ...
            # 次ステップ視覚も scale_vision で [-1,1] に変換
            ...
            # pred['self_vision'] は [-1,1]、v_next_t も [-1,1] → 一致した空間で損失計算
    ...
    torch.save({
        ...
        'scale_vision':         True,    # このフラグで mg3/mg4 を区別
        'mg_input':             'ov_enc',
