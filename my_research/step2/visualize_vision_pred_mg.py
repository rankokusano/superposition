"""
visualize_vision_pred_mg.py

Motion Generator 版：視覚予測の比較図
visualize_vision_pred.py を保持したまま、新設計として独立したファイル。

visualize_vision_pred.py からの変更点:
    1. SuperpositionNetworkWithMG を使用
...
使い方:
    ...
    MESA_GL_VERSION_OVERRIDE=3.3 xvfb-run python3 visualize_vision_pred_mg.py \\
        --run mg_20260601_201025

    # パス直接指定も可
    MESA_GL_VERSION_OVERRIDE=3.3 xvfb-run python3 visualize_vision_pred_mg.py \\
        --run /work/my_research/step2/results/mg_20260601_201025

出力:
    {run_dir}/vision_pred_mg.png   ← 比較図
    標準出力にピクセル誤差 (MAE) を表示
"""
...
MODEL_FILE = os.path.join(RUN_DIR, 'model_mg.pth')
SAVE_PATH  = os.path.join(RUN_DIR, 'vision_pred_mg.png')
