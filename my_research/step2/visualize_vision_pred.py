"""
visualize_vision_pred.py

視覚予測の比較図

上段: A-1側（ネットワークの学習目標そのもの）
  Row 1: A-1 の入力視覚 v_t
  Row 2: ネットワークの予測 v̂_{t+1}   ← pred['self_vision']
  Row 3: A-1 の正解 v_{t+1}           ← 実際の次フレーム

下段: A-2側（視点取得の確認）
  Row 4: f²_v をデコードした画像       ← Process-2 が「見ている」もの
  Row 5: A-2 の正解視野               ← シミュレーションで直接取得

Row 2 ≈ Row 3 → ネットワークが視覚予測を学習できている
Row 4 ≈ Row 5 → Process-2 が A-2 の視点を獲得している（視点取得）

使い方:
    # v2 モデル（デフォルト）
    MESA_GL_VERSION_OVERRIDE=3.3 xvfb-run python3 visualize_vision_pred.py

    # v3 モデル（全体ファインチューニング後）
    MESA_GL_VERSION_OVERRIDE=3.3 xvfb-run python3 visualize_vision_pred.py --model v3

出力:
    step2/vision_pred_v2.png  または  step2/vision_pred_v3.png
"""
...
MODEL_FILE = {
    'v2': os.path.join(_HERE, 'model_q.pth'),
    'v3': os.path.join(_HERE, 'model_q_v3.pth'),
}[args.model]
SAVE_PATH = os.path.join(_HERE, f'vision_pred_{args.model}.png')
...
DECODER_EPISODES = 20    # デコーダー学習用データ収集エピソード数
DECODER_STEPS    = 60
DECODER_EPOCHS   = 30
N_SAMPLES        = 5     # 表示するタイムステップ数
SAMPLE_INTERVAL  = 8
Q_SCALE          = 1.0
...
# =========================================================================
# サンプル収集（1エピソード時系列）
# A-1: 赤から遠い右下(7,-7)スタート → SACで赤(-9,+9)へ向かう過程を記録
# A-2: 緑から遠い右上(7,+7)スタート → GreenFollowerで緑(-9,-9)へ向かう過程を記録
# =========================================================================
SAMPLE_STEPS = [5, 15, 25, 35, 45]   # 各列に使うタイムステップ
TOTAL_STEPS  = max(SAMPLE_STEPS) + 1

print(f"\nサンプル収集（1エピソード時系列: step={SAMPLE_STEPS}）...")

start_a1 = np.array([ 7.0, -7.0])   # 右下: 赤(-9,+9)から遠い
start_a2 = np.array([ 7.0,  7.0])   # 右上: 緑(-9,-9)から遠い
...
for col, s in enumerate(samples):
    ...
    for row, (img_t, (bg, _, _)) in enumerate(zip(imgs, row_info)):
        ...
        if col == 0:
            _, long_label, short_label = row_info[row]
...
title_suffix = "(v3: 全体ファインチューニング)" if args.model == 'v3' else "(v2: VisionEncoder固定)"
fig.suptitle(
    f'視覚予測の比較  {title_suffix}\n'
    '②≈③ならA-1の予測成功、④≈⑤なら視点取得成功',
    fontsize=11, y=0.995
)
...
print(f"  A-2 視点 (④ vs ⑤) : {err_dec:.4f}  ← 視点取得")
print()
