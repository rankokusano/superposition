...
マスキング修正版 Step2 学習スクリプト。
train_step2_mg2.py からの変更点:

    p_mask_vision_other: 0.0 → (step==0 ? 0.0 : 1.0)

    元論文の MotionGenerationPredictionRunner に準拠:
        t=0  : マスクなし（LSTM に ov_enc が見える）
        t>0  : other 側を完全マスク（LSTM は MG 出力だけを頼りにする）

    この圧力によって:
        - MG がほぼゼロを出力する問題を解消
        - other_enc が A-2 の視点情報を ov_enc に強く表現するよう誘導
        - 学習後半での loss 増加を防ぐ

    保存先: results/mg3_YYYYMMDD_HHMMSS/

実行コマンド:
    MESA_GL_VERSION_OVERRIDE=3.3 xvfb-run python3 train_step2_mg3.py
"""
...
RUN_DIR         = os.path.join(_HERE, 'results', f'mg3_{_TIMESTAMP}')
MODEL_SAVE_PATH = os.path.join(RUN_DIR, 'model_mg3.pth')
