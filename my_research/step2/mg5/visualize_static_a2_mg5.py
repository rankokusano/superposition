"""
visualize_static_a2_mg5.py

mg5 モデル（encoder freeze 済み）× 静止A-2 で視点取得を確認。
追加学習なし。

確認したいこと:
  exp1 style（静止A-2）でも Row④≠Row⑤ → encoder 自体がまだおかしい
  exp1 style（静止A-2）で Row④≈Row⑤ → encoder は正常、問題は動くA-2への適応

exp1+exp2 との違い:
  モデル: exp1 オリジナル → mg5（encoder freeze 後）
  デコーダ: exp2 学習済み → 新鮮学習（同じ条件で比較）
  A-2: 静止（ランダム配置、試行内は固定）← exp1 と同じ

実行コマンド:
    MESA_GL_VERSION_OVERRIDE=3.3 xvfb-run python3 visualize_static_a2_mg5.py \\
        --run mg5_YYYYMMDD_HHMMSS

出力:
    {run_dir}/vision_pred_static_a2.png
"""
...
SAVE_PATH  = os.path.join(RUN_DIR, 'vision_pred_static_a2.png')
...
DECODER_EPISODES = 30
DECODER_STEPS    = 80
DECODER_EPOCHS   = 50   # exp2 に近い品質を目指して多めに
N_SAMPLES        = 5
...
print(f"  encoder_frozen={ckpt.get('encoder_frozen', '?')}  "
...
# =========================================================================
# decoder 学習（self_enc → 再構成）
# =========================================================================
print(f"decoder 学習 ({DECODER_EPISODES} ep × {DECODER_STEPS} steps × {DECODER_EPOCHS} epochs)...")
frames = []
for ep in range(DECODER_EPISODES):
    env.reset()
    v_raw, _, _, sp, op = env.step()
    actor_h = None
    # A-2 は env.reset() の位置に固定
    fixed_op = op.copy()
    for _ in range(DECODER_STEPS):
        v_t = to_scaled(v_raw)
        frames.append(v_t.squeeze(0).cpu())
        with torch.no_grad():
            action, _, actor_h = actor.sample(to_raw(v_raw), actor_h)
        a_np = action.squeeze(0).cpu().numpy()
        sp   = np.clip(sp + a_np, -9.5, 9.5)
        env.self_agent.p  = sp
        env.other_agent.p = fixed_op   # A-2 を動かさない
...
for epoch in range(DECODER_EPOCHS):
    ...
    for s in range(0, N, 32):
        ...
        loss = F.l1_loss(decoder(f1), batch)
...
for step in range(max(SAMPLE_STEPS) + 2):
    ...
    env.other_agent.p = pos_a2     # A-2 は固定
...
for col, s in enumerate(samples):
    ...
    for row, img_t in enumerate(imgs):
        ...
        for sp in ax.spines.values():
            sp.set_edgecolor('#6c3483' if row >= 3 else '#999')
            sp.set_linewidth(1.5 if row >= 3 else 0.8)
        if col == 0:
...
fig.suptitle(
    f'mg5 × 静止A-2 視点取得確認  [{run_label}]\n'
    f'A-2 固定位置: ({pos_a2[0]:.1f}, {pos_a2[1]:.1f})  '
    f'A-1 は SAC で移動\n'
    f'④≈⑤ → encoder 正常（問題は動くA-2への適応）\n'
    f'④≠⑤ → encoder 自体に問題あり',
    fontsize=9, y=0.999
)
...
print(f"\n=== MAE ===")
print(f"  A-1 予測 (②vs③) : {err_pred:.4f}  baseline={err_base:.4f}  → {'OK' if err_pred < err_base else 'NG'}")
print(f"  A-2 視点 (④vs⑤) : {err_dec:.4f}  baseline={err_base:.4f}  → {'OK' if err_dec < err_base else 'NG'}")
print()
if err_dec < err_base:
    print("→ encoder は正常。問題は「動くA-2への適応」→ ② (0.99 masking) へ進む")
else:
    print("→ 静止A-2でも視点取得が不十分。encoder または decoder に課題あり")
