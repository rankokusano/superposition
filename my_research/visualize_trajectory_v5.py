...
from rl_agent_lstm import DQNAgentLSTM
...
model = DQNAgentLSTM().to(DEVICE)
model.load_state_dict(torch.load('/work/my_research/rl_model_v5.pth'))
model.eval()
...
for ep in range(n_episodes):
    ...
    for step in range(100):
        action, action_idx, hidden = model.get_action(state, hidden, epsilon=0.0)
...
plt.suptitle('Agent Trajectory v5 (CNN+LSTM, 20 episodes, epsilon=0)', fontsize=14)
plt.tight_layout()
plt.savefig('/work/my_research/trajectory_v5_20ep.png', dpi=100, bbox_inches='tight')
print("Saved: trajectory_v5_20ep.png")
