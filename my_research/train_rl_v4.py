...
from rl_agent_discrete import DQNAgentDiscrete, ACTIONS, N_ACTIONS
...
def train():
    ...
    target_model = DQNAgentDiscrete().to(DEVICE)
    ...
    for episode in range(EPISODES):
        ...
        for step in range(MAX_STEPS):
            action, action_idx = model.get_action(state, epsilon)
            ...
            if len(memory) >= BATCH_SIZE:
                ...
                action_idxs = torch.LongTensor(action_idxs).to(DEVICE)
                rewards = torch.FloatTensor(rewards).to(DEVICE)
                dones = torch.FloatTensor(dones).to(DEVICE)

                # 現在のQ値（選んだ行動のQ値だけ取り出す）
                q_values = model(states).gather(1, action_idxs.unsqueeze(1)).squeeze(1)
    ...
    torch.save(model.state_dict(), '/work/my_research/rl_model_v4.pth')
    print("学習完了: rl_model_v4.pth を保存しました")
