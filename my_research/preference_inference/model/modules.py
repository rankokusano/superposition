...
class ValueEstimatorModule(nn.Module):
    """
    Approach B: Estimates Q̂² from ov_enc (64-dim) + om_generated (2-dim).
    Architecture: [ov_enc, om_gen] → Linear → ReLU → Linear → ReLU → Linear → Q̂²(1-dim)
    No RNN state: stateless feedforward.
    """
    def __init__(self, config):
        super().__init__()
        enc_dim = config.input.vision
        motion_dim = config.input.motion
        hidden = config.hidden

        self.net = nn.Sequential(
            nn.Linear(enc_dim + motion_dim, hidden),
            nn.ReLU(),
        ...
        for layer in self.net:
            if isinstance(layer, nn.Linear):
                util.init_fc(layer, 'relu', 'he')

    def forward(self, ov_enc, om_generated):
