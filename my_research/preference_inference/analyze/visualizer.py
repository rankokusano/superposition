...
LANDMARK_COLORS = ['red', 'green', 'blue', 'gold']
...
class Visualizer(object):
    ...
    def save_pdf(self, f_base):
        self.fig.savefig(f_base + '.pdf', rasterized=True, dpi=300)
    ...
    def plot(self, hidden, position, plot_type, lim, landmark_labels=None):
        ds = []
        for n in range(hidden.shape[0]):
            h = hidden[n, :]
            p = position[n, :]
            lm = landmark_labels[n, :] if landmark_labels is not None else None
    ...
    def plot_hidden(self, hidden, position, plot_type, lim=None, landmark_labels=None):
        p1 = hidden[:, 0]
        p2 = hidden[:, 1]
        ...
        if plot_type in ['self_position', 'other_position']:
            t1 = position[:, 0]
            t2 = position[:, 1]
        ...
        elif plot_type == 'a2_landmark':
            # Color by A-2 target landmark (4 classes)
            assert landmark_labels is not None, "landmark_labels required for a2_landmark plot"
            colors = [LANDMARK_COLORS[int(lbl) % len(LANDMARK_COLORS)]
                      for lbl in landmark_labels]
            d = self.ax.scatter(p1, p2, c=colors, alpha=0.5, zorder=0, s=4)
        elif plot_type == 'point':
            d = self.ax.scatter(p1, p2, c='w', edgecolor='k', s=200, zorder=0)
        else:
            assert False, f"Unknown plot_type: {plot_type}"
        return d
