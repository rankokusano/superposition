    ...
    def run_seq(self, model, seq_loader, epoch, config, collect_data=False):
        ...
        for (x, truth, t,) in seq_loader:
            ...
            if collect_data:
                ...
                # A-2 target landmark label (for analysis)
                if "a2_target_landmark" in x:
                    _data["a2_target_landmark"] = {"input": x["a2_target_landmark"]}

                # Approach B: collect q2_hat and other_motion (om_generated)
                if "q2_hat" in pred:
...
class MotionGenerationPredictionRunner(PredictionRunnerBase):
    def gen_mask_prob(self, t, config):
...
class MotionGenerationPredictionFeaturePredictionRunner(
    MotionGenerationPredictionRunner
):
    def calc_loss(self, model, loss, pred, truth, epoch, t, config):
        super().calc_loss(model, loss, pred, truth, epoch, t, config)
...
# ---------------------------------------------------------------------------
# Approach B runners (new)
# ---------------------------------------------------------------------------

class ApproachBBaseRunner(PredictionRunnerBase):
    """
    Runner for SuperpositionNetworkApproachBBase.
    SM uses 1-dim Q-value input. Masking same as PredictionRunner.
    """
    def gen_mask_prob(self, t, config):
        return 0 if t == 0 else config.p_mask_vision


class ApproachBMGVERunner(PredictionRunnerBase):
    """
    Runner for SuperpositionNetworkApproachBMGVE.
    Always masks (like MotionGenerationPredictionRunner).
    Loss = visual prediction only (VE trained indirectly).
    """
    def gen_mask_prob(self, t, config):
        return 0 if t == 0 else 1


class ApproachBBaseV3Runner(ApproachBBaseRunner):
    """
    B-base v3 runner with FPM loss.
    ss/os are detached before FPM so FPM learns to map hidden → features
    without forcing SM to restructure its hidden states for FPM's benefit.
    SM is shaped by vision reconstruction loss only.
    """
    def calc_loss(self, model, loss, pred, truth, epoch, t, config):
        super().calc_loss(model, loss, pred, truth, epoch, t, config)
        ...
        pred_self_feature = model.predict_feature(ss.detach())
        pred_other_feature = model.predict_feature(os.detach())
        loss["feature_prediction_self"] += F.mse_loss(pred_self_feature, sv_enc.detach())
        loss["feature_prediction_other"] += F.mse_loss(pred_other_feature, ov_enc.detach())


class ApproachBMGVEV3Runner(ApproachBMGVERunner):
    """
    B-MGVE v3 runner with FPM loss.
    ss/os are NOT detached: gradient flows FPM → os → SM (frozen) → MG/VE.
    This is the primary indirect learning signal for MG and VE.
    """
    def calc_loss(self, model, loss, pred, truth, epoch, t, config):
        super().calc_loss(model, loss, pred, truth, epoch, t, config)
